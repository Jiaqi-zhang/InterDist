...
def preprocess_plot_motion(motions, caption, vis_dir, npy_dir, file_name, foot_ik=False):
    ...
    for j in range(2):
        ...
        if foot_ik:
            ik_joint, foot = remove_fs(joints3d, None, fid_l=(7, 10), fid_r=(8, 11), interp_length=5, force_on_floor=True)
    ...
    plot_3d_motion(pjoin(vis_dir, file_name + f".mp4"), paramUtil.t2m_kinematic_chain, sequences, title=caption, fps=30)
    if foot_ik:
        plot_3d_motion(pjoin(vis_dir, "ik_" + file_name + f".mp4"), paramUtil.t2m_kinematic_chain, sequences_ik, title=caption, fps=30)
    return
...
def plot_3d_motion_2views(save_path, kinematic_tree, mp_joints, title, figsize=(20, 10), fps=120, radius=8, foots=None):
    ...
    colors = ['orange', 'green', 'black', 'red', 'blue', 'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkred', 'darkred', 'darkred', 'darkred', 'darkred']
        ...
        for pid, data in enumerate(mp_data):
            for i, (chain, color) in enumerate(zip(kinematic_tree, mp_colors[pid])):
                ...
                if foots is not None:
                    ...
                    if l_toe == 1:
                        color = 'darkred'
                axs[0].plot3D(data["joints"][index, chain, 0], data["joints"][index, chain, 1], data["joints"][index, chain, 2], linewidth=linewidth, color=color)
                axs[1].plot3D(data["joints_rot"][index, chain, 0], data["joints_rot"][index, chain, 1], data["joints_rot"][index, chain, 2], linewidth=linewidth, color=color)
...
def plot_3d_motion(save_path, kinematic_tree, mp_joints, title, figsize=(10, 10), fps=120, radius=4):
    ...
    #
    colors = ['red', 'green', 'black', 'red', 'blue', 'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkred', 'darkred', 'darkred', 'darkred', 'darkred']
        ...
        for pid, data in enumerate(mp_data):
            for i, (chain, color) in enumerate(zip(kinematic_tree, mp_colors[pid])):
                ...
                if i < 5:
                    ...
                    linewidth = 1.0
                ax.plot3D(data["joints"][index, chain, 0], data["joints"][index, chain, 1], data["joints"][index, chain, 2], linewidth=linewidth, color=color)
