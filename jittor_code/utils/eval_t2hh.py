...
def calculate_top_k(mat, top_k):
    ...
    correct_vec = False
    top_k_list = []
    valid_k = min(top_k, bool_mat.shape[1])
    for i in range(valid_k):
        #         print(correct_vec, bool_mat[:, i])
        correct_vec = (correct_vec | bool_mat[:, i])
        # print(correct_vec)
        top_k_list.append(correct_vec[:, None])
    for _ in range(valid_k, top_k):
        top_k_list.append(correct_vec[:, None])
...
def _safe_diversity_times(nb_sample, requested):
    return max(1, min(int(requested), int(nb_sample) - 1))
...
def predict_interx(joint_dist, motions, motion_lens, net, dataset):
    ...
    pred_pose_eval = np.zeros((bs, t, d), dtype=np.float32)
    for i in range(bs):
        s_seq = int(motion_lens[i].item()) if hasattr(motion_lens[i], "item") else int(motion_lens[i])
        ...
        pred_pose_eval[i:i + 1, :s_seq, :d] = pred_pose

    pred_pose_eval = jt.from_numpy(pred_pose_eval).cuda()
...
def predict_inter_human(motion_norm, joint_dist, net, motion_lens, dataset):
    ...
    for i in range(bs):
        ...
        pred_pose1, pred_pose2 = net(
            motion_norm[i:i + 1, :s_seq],
            joint_dist[i:i + 1, :s_seq],
        )[:2]
        pred_pose1 = pred_pose1.cpu().numpy()
        pred_pose2 = pred_pose2.cpu().numpy()
        ...
        pred_pose2 = dataset.normalizer.backward(pred_pose2)
        pred_pose = np.concatenate([pred_pose1, pred_pose2], axis=-1)
        pred_pose_eval[i:i + 1, :s_seq, :dim] = pred_pose

    pred_mot1, pred_mot2 = pred_pose_eval[..., :dim // 2], pred_pose_eval[..., dim // 2:]
    pred_mot1, pred_mot2 = jt.from_numpy(pred_mot1), jt.from_numpy(pred_mot2)
...
def evaluation_vqvae(
    ...
    for batch in val_loader:
        if dataname == 'InterHuman':
            ...
            et_np, em_np = et.cpu().numpy(), em.cpu().numpy()
            del et, em
            jt.gc()
    ...
    motion_annotation_np = jt.cat(motion_annotation_list, dim=0).cpu().numpy()
    motion_pred_np = jt.cat(motion_pred_list, dim=0).cpu().numpy()
    ...
    if fid < best_fid:
        ...
        if save:
            jt.save({'net': net.state_dict(), 'nb_iter': nb_iter}, os.path.join(out_dir, 'net_best_fid.pth'))
    ...
    if save:
        jt.save({'net': net.state_dict(), 'nb_iter': nb_iter}, os.path.join(out_dir, 'net_last.pth'))
...
def evaluation_vqvae_test(
    ...
    div_times = _safe_diversity_times(nb_sample, diversity_times)
    diversity_real = calculate_diversity(motion_annotation_np, div_times, EMB_SCALE, DIV_SCALE)
    diversity = calculate_diversity(motion_pred_np, div_times, EMB_SCALE, DIV_SCALE)
...
def predict_t2m_interx(motions, motion_lens, vq_model, trans, dataset, caption, m_tokens_len, time_steps, cond_scale):
    ...
    token_lens = [int(m_tokens_len[i].item()) for i in range(len(m_tokens_len))]
    max_len = max(token_lens)
    pred_ids_lst = trans.generate(caption, m_tokens_len, time_steps, cond_scale, max_len=max_len, temperature=1)
    ...
    pred_pose_eval = np.zeros((bs, mot_length, d), dtype=np.float32)
    for k in range(bs):
        s_seq = int(motion_lens[k].item()) if hasattr(motion_lens[k], "item") else int(motion_lens[k])

        ######### [INFO] Eval by m_length
        sm1_ids = reformat_token(m1_pred_ids[k:k + 1], token_lens[k])
        sm2_ids = reformat_token(m2_pred_ids[k:k + 1], token_lens[k])
        sd_ids = reformat_token(dist_pred_ids[k:k + 1], token_lens[k])
        ...
        pred_pose = np.concatenate([pred_pose1, pred_pose2], axis=-1)
        pred_pose = pred_pose.reshape(1, s_seq, -1)
        pred_pose_eval[k:k + 1, :s_seq, :] = pred_pose

    pred_pose_eval = jt.from_numpy(pred_pose_eval).cuda()
    return pred_pose_eval


def predict_t2m_inter_human(motions, motion_lens, vq_model, trans, dataset, text, m_tokens_len, time_steps, cond_scale):
    token_lens = [int(m_tokens_len[i].item()) for i in range(len(m_tokens_len))]
    max_len = max(token_lens)
    ...
    for k in range(bs):
        ...
        pred_pose1 = dataset.normalizer.backward(pred_pose1)
        pred_pose2 = dataset.normalizer.backward(pred_pose2)
        pred_pose_eval1[k:k + 1, :s_seq, :] = pred_pose1
        pred_pose_eval2[k:k + 1, :s_seq, :] = pred_pose2

    pred_pose_eval1 = jt.from_numpy(pred_pose_eval1).cuda()
    pred_pose_eval2 = jt.from_numpy(pred_pose_eval2).cuda()
...
def evaluation_transformer_mask_test(
        ...
        if dataname == 'InterHuman':
            ...
            if i < num_mm_batch:
                ...
                for _ in range(mm_num_repeats):
                    pred_pose_eval1, pred_pose_eval2 = predict_t2m_inter_human(
                        motion1, motion_lens, vq_model, trans, dataset, text,
                        m_tokens_len, time_steps, cond_scale,
                    )
                    
                    et_pred, em_pred = eval_wrapper.get_co_embeddings([name, text, pred_pose_eval1, pred_pose_eval2, motion_lens])
                    motion_multimodality_batch.append(em_pred.unsqueeze(1))
                motion_multimodality_batch = jt.cat(motion_multimodality_batch, dim=1)  #(bs, 30, d)
    ...
    multimodality = 0
    if not force_mask and cal_mm and motion_multimodality:
        motion_multimodality = jt.cat(motion_multimodality, dim=0).cpu().numpy()
        if motion_multimodality.shape[1] > 1:
            multimodality = calculate_multimodality(motion_multimodality, min(mm_num_times, motion_multimodality.shape[1] - 1))
...
def predict_t2m_inter_human_reaction(motion_norm, joint_dist, motions, motion_lens, vq_model, trans, dataset, text, m_tokens_len, time_steps, cond_scale):
    ...
    bs = motion_norm.shape[0]
    m1_gt_ids = jt.ones((bs, nbp, max_len)).cuda().long() * pad_id  # [96, 5, 74]
    motion_norm = jt.tensor(motion_norm).cuda().float()
    joint_dist = jt.tensor(joint_dist).cuda().float()
    for k in range(bs):
...
def predict_t2m_inter_human_inbetween(motion_norm, joint_dist, motions, motion_lens, vq_model, trans, dataset, text, m_tokens_len, time_steps, cond_scale):
    ...
    bs = motion_norm.shape[0]
    m1_gt_ids = jt.ones((bs, nbp, max_len)).cuda().long() * pad_id  # [96, 5, 74]
    m2_gt_ids = jt.ones((bs, nbp, max_len)).cuda().long() * pad_id  # [96, 5, 74]
    sd_gt_ids = jt.ones((bs, nbp, max_len)).cuda().long() * pad_id  # [96, 5, 74]
    edit_mask = jt.zeros((bs, nbp, max_len)).cuda().bool()
