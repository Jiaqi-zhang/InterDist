...
class InterxKinematics():
    ...
    def forward(self, motions, mode='fk'):
        """
        Args:
            motions: jt.Tensor of shape (B, T, 56, dim) 
            T=64 for vqvae
            dim=6 for 6d rots
        """
        B, T, J, dim = motions.shape
        ...
        if mode == 'fk':
            ...
            # jt.Size([256, 32, 3]) jt.Size([256, 32, 54, 3])
            body_pose = jt.cat(
                [root_orient[:, :, None, :], body_pose], dim=2)  # (B, T, 1+54, 3)
            motions_pos = self.forward_kinematics(root_trans, body_pose)
        elif mode == 'bm':
            motions_pos = []
            # jt.zeros(B, T, J-1, 3, requires_grad=True).to(motions.device)
            for b in range(B):
            ...
            motions_pos = jt.stack(motions_pos, dim=0)
        else:
    ...
    def forward_kinematics(self, root_pos, pose):
        '''  
            root_pos: jt.Tensor of shape (B, T, 3)  
            pose: jt.Tensor of shape (B, T, 55, 3)  
            offsets: jt.Tensor of shape (55, 3)  

            return:
            global_positions: jt.Tensor of shape (B, T, 55, 3)
        '''
        b, t, j, _ = pose.shape
        ...
        offsets = jt.from_numpy(self.offsets).to(joints_rotations.device)
        ...
        for i in range(1, j):
            ...
            global_rot_i = jt.matmul(parent_rot, joints_rotations[:, :, i])
            global_offset = jt.matmul(
        ...
        global_positions = jt.stack(global_positions_list, dim=2)
