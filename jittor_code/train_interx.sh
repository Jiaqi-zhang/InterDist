...
if [[ "$STAGE" != "all" && "$STAGE" != "vq" && "$STAGE" != "t2m" ]]; then
  echo "Usage: DATA_ROOT=/absolute/path/to/InterX bash $0 [all|vq|t2m]" >&2
  exit 2
fi

if [[ -z "${DATA_ROOT:-}" ]]; then
  echo "DATA_ROOT must point to the Inter-X dataset root." >&2
  exit 2
fi
if [[ ! -d "$DATA_ROOT" ]]; then
...
VQ_EXP_NAME="${VQ_EXP_NAME:-inter_x_vq_model}"
VQ_BATCH_SIZE="${VQ_BATCH_SIZE:-128}"
VQ_TOTAL_ITER="${VQ_TOTAL_ITER:-150000}"
VQ_LR="${VQ_LR:-1e-4}"
VQ_LR_SCHEDULER="${VQ_LR_SCHEDULER:-120000}"

T2M_EXP_NAME="${T2M_EXP_NAME:-inter_x_t2m_model}"
T2M_BATCH_SIZE="${T2M_BATCH_SIZE:-48}"
T2M_TOTAL_ITER="${T2M_TOTAL_ITER:-200000}"
T2M_LR="${T2M_LR:-2e-4}"
T2M_LR_SCHEDULER="${T2M_LR_SCHEDULER:-50000}"
VQ_MODEL_PTH="${VQ_MODEL_PTH:-./results/InterX/$VQ_EXP_NAME/net_best_fid.pth}"
