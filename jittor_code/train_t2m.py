...
from utils.jittor_compat import clip_grad_norm_
from utils.jittor_logging import SummaryWriter
...
def get_acc(cls_pred, target, mask):
    cls_pred = jt.masked_select(cls_pred, mask.unsqueeze(-1)).view(-1, cls_pred.shape[-1])
    target_all = jt.masked_select(target, mask)
    probs = jt.softmax(cls_pred, dim=-1)
    _, cls_pred_index = jt.max(probs, dim=-1)
...
def load_vq_model(opt):
    ...
    ckpt = jt.load(opt.vq_model_pth, map_location='cpu')
...
jt.autograd.set_detect_anomaly(True)  # Enable PyTorch anomaly detection
...
if opt.dataname == 'InterHuman':
    ...
    if opt.data_root is not None:
        data_cfg.data_root = opt.data_root
    if opt.eval_model_pth is not None:
        eval_model_cfg.checkpoint_path = opt.eval_model_pth
    if opt.eval_batch_size is not None:
        eval_model_cfg.batch_size = opt.eval_batch_size
    ...
    from options.option_data import configure_interx_paths
    ...
    ##### ---- Dataloader ---- #####
    configure_interx_paths(data_cfg, eval_model_cfg, opt.data_root, opt.eval_model_pth)
    if opt.eval_batch_size is not None:
        eval_model_cfg.batch_size = opt.eval_batch_size
    w_vectorizer = WordVectorizer(pjoin(data_cfg.data_root, 'glove'), 'hhi_vab')
    eval_wrapper = EvaluatorModelWrapper(opt.device, eval_model_cfg.checkpoints_dir, eval_model_cfg.checkpoint_path)
...
if opt.resume_trans is not None:
    ckpt = jt.load(opt.resume_trans, map_location='cpu')
...
scheduler = jt.optim.lr_scheduler.MultiStepLR(optimizer, milestones=opt.lr_scheduler, gamma=opt.gamma)
...
for nb_iter in tqdm(range(start_nb_iter, opt.total_iter + 1), position=0, leave=True):
    ...
    conds = clip_text.cuda().float() if jt.is_tensor(clip_text) else clip_text
