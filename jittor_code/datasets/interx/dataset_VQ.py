...
from utils import jittor_data as data
...
class VQMotionDataset(data.Dataset):
    ...
    def to_rot_6d(self, data):
        ...
        ret = jt.cat(pose_all, dim=2)
        padded_tr = jt.zeros((ret.shape[0], ret.shape[2]), dtype=ret.dtype)
        for ii in range(self.num_person):
            padded_tr[:, 6 * ii:6 * ii + 3] = ret_tr[:, 3 * ii:3 * ii + 3]
        ret = jt.cat((ret, padded_tr[:, None]), 1)
...
def DATALoader(data_cfg, batch_size, window_size, num_workers=8, shuffle=True):
    ...
    train_loader = data.DataLoader(trainSet, batch_size, shuffle=shuffle, num_workers=num_workers, drop_last=True)
