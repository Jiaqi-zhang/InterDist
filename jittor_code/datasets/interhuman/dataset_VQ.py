...
class VQMotionDataset(data.Dataset):
    ...
    def __init__(self, opt, window_size, normalize=True):
        ...
        ignore_path = os.path.join(self.data_root, "./../split/ignore_list.txt")
        ignore_list = []
        if os.path.exists(ignore_path):
            ignore_list = [item.strip() for item in open(ignore_path, "r").readlines()]
...
def DATALoader(data_cfg, batch_size, window_size, num_workers=8):
    ...
    train_loader = data.DataLoader(trainSet, batch_size, shuffle=True, num_workers=num_workers, drop_last=True)
