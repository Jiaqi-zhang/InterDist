...
def lengths_to_mask(lengths, max_len):
    # max_len = max(lengths)
    mask = jt.arange(max_len, device=lengths.device).expand(len(lengths), max_len) < lengths.unsqueeze(1)
    return mask  #(b, len)
...
def get_subsequent_mask(seq):
    ...
    subsequent_mask = (1 - jt.triu(jt.ones((1, seq_len, seq_len)), diagonal=1)).bool()
...
def uniform(shape, device=None):
    return jt.zeros(shape, device=device).float().uniform_(0, 1)


def prob_mask_like(shape, prob, device=None):
    if prob == 1:
        return jt.ones(shape, device=device, dtype=jt.bool)
    elif prob == 0:
        return jt.zeros(shape, device=device, dtype=jt.bool)
    else:
...
def log(t, eps=1e-20):
    return jt.log(t.clamp(min=eps))


def gumbel_noise(t):
    noise = jt.zeros_like(t).uniform_(0, 1)
...
def top_k(logits, thres=0.9, dim=1):
    ...
    probs = jt.full_like(logits, float('-inf'))
    probs.scatter_(dim, ind, val)
    return probs

# More on large value, less on small
def cosine_schedule(t):
    return jt.cos(t * math.pi * 0.5)


def scale_cosine_schedule(t, scale):
    return jt.clip(scale * jt.cos(t * math.pi * 0.5) + 1 - scale, min=0., max=1.)
...
def q_schedule(bs, low, high, device):
    ...
    return jt.round(schedule * (high - low - 1)).long() + low
...
def cal_performance(pred, labels, ignore_index=None, smoothing=0., tk=1):
    ...
    pred_id_k = jt.topk(pred, k=tk, dim=1).indices
    pred_id = pred_id_k[:, 0]
    mask = labels.ne(ignore_index)
    n_correct = (pred_id_k == labels.unsqueeze(1)).any(dim=1).masked_select(mask)
    acc = jt.mean(n_correct.float()).item()
...
def cal_loss(pred, labels, ignore_index=None, smoothing=0.):
    ...
    if smoothing:
        ...
        n_class = pred.size(1)
        mask = labels.ne(ignore_index)
        one_hot = F.one_hot(labels, n_class + space)
        dims = list(range(len(one_hot.shape)))
        one_hot = one_hot.permute(0, dims[-1], *dims[1:-1])[:, :n_class]
        # one_hot = jt.zeros_like(pred).scatter(1, labels.unsqueeze(1), 1)
        ...
        loss = jt.mean(loss.masked_select(mask))
    else:
        # cross_entropy + dice
...
def cal_dice_loss(pred, target, ignore_index=None, smooth=1e-5):
    ...
    Args:  
        pred (jt.Tensor): Model output [batch_size, num_classes, num_features]  
        target (jt.Tensor): Ground truth labels [batch_size, num_features]  
    ...
    intersection = jt.sum(pred * target_one_hot, dim=-1)
    pred_sum = jt.sum(pred, dim=-1)
    target_sum = jt.sum(target_one_hot, dim=-1)
...
def cal_performance_two(
    ...
    # Get predictions
    pred_id_first_k = jt.topk(logits_first, k=tk, dim=1).indices
    pred_id_first = pred_id_first_k[:, 0]  # (bs, 55)
    pred_id_second_k = jt.topk(logits_second, k=tk, dim=1).indices
