...
from models.positional_encodings import PositionalEncoding2D


def _stop_fuse_list(values):
    return [value.stop_fuse() for value in values]
...
class OutputProcess(nn.Module):
    ...
    def execute(self, hidden_states: jt.Tensor, cond: jt.Tensor) -> jt.Tensor:
...
class MaskTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.tags_token = nn.Parameter(jt.zeros(3, self.embed_dim))
    ...
    def initialize_weights(self):
        ...
        def __init_weights(module):
            if isinstance(module, (nn.Linear, nn.Embedding)):
                module.weight.assign(jt.randn(module.weight.shape) * 0.02)
                if isinstance(module, nn.Linear) and module.bias is not None:
                    module.bias.assign(jt.zeros_like(module.bias))
            elif isinstance(module, nn.LayerNorm):
                if module.bias is not None and hasattr(module.bias, "assign"):
                    module.bias.assign(jt.zeros_like(module.bias))
                if module.weight is not None and hasattr(module.weight, "assign"):
                    module.weight.assign(jt.ones_like(module.weight))
    ...
    def mask_cond(self, cond, force_mask=False):
        bs = cond.shape[0]
        if force_mask:
            return jt.zeros_like(cond)
        elif self.training and self.cond_drop_prob > 0.:
            if len(cond.shape) == 2:
                mask = jt.bernoulli(jt.ones(bs, device=cond.device) * self.cond_drop_prob).view(bs, 1)
            else:
                mask = jt.bernoulli(jt.ones(bs, device=cond.device) * self.cond_drop_prob).view(bs, 1, 1)
            return cond * (1. - mask)
        else:
            return cond
    ...
    def trans_forward(self, ids_lst, cond, cond_word, padding_mask, force_mask=False):
        ...
        bs = bs // self.nbp
        x_ids = jt.cat(ids_lst, dim=1)  # (b*j*3, seqlen)
        ...
        cond_tag = jt.cat((cond_tag, tag_tokens), dim=-1)  # (bs, 3, latent_dim + embed_dim)
    ...
    def _mask_tokens(self, x_ids, mask):
        ...
        rand_id = jt.randint_like(x_ids, high=self.num_tokens)
        x_ids = jt.where(mask_rid, rand_id, x_ids)
        # Step 2: 90% x 10% replace with correct token, and 90% x 88% replace with mask token
        mask_mid = get_mask_subset_prob(mask & ~mask_rid, 0.88)
        x_ids = jt.where(mask_mid, self.mask_id, x_ids)
        return x_ids

    def _prepare_tokens(self, ids, bs, device, ntokens, non_pad_mask):
        # Positions that are PADDED are ALL FALSE
        ids = jt.where(non_pad_mask, ids, self.pad_id)
        ...
        batch_randperm = jt.rand((bs, ntokens), device=device).argsort(dim=-1)
        ...
        labels = jt.where(mask, ids, self.mask_id)
    ...
    def _prepare_tokens_part(self, ids, non_pad_mask):
        ...
        # ! num of local motion tokens == num of global motion tokens
        rand_id = jt.randint_like(motion_ids, high=self.num_tokens)
        motion_ids = jt.where(mask_rid, rand_id, motion_ids)
        # Step 2: 90% x 10% replace with correct token, and 90% x 88% replace with mask token
        mask_mid = mask[:, 0, :] & ~mask_rid[:, 0, :]
        mask_mid = get_mask_subset_prob(mask_mid, 0.88)
        mask_mid = mask_mid[:, None, :].repeat(1, nbp, 1)  # (b, 5, t)
        motion_ids = jt.where(mask_mid, self.mask_id, motion_ids)
        return motion_ids, labels

    def execute(self, m1_ids, m2_ids, d_ids, y, m_lens):
        '''
        :param ids_mot: (b, n)
        ...
        if self.cond_mode == 'text':
            ...
            cond_vector = jt.zeros(bs, self.latent_dim).float().to(device)
            cond_word_vector = jt.zeros(bs, 77, self.latent_dim).float().to(device)
            force_mask = True
        else:
    ...
    def forward_with_cond_scale(self, ids_lst, cond_vector, cond_word_vector, padding_mask, cond_scale=3, force_mask=False):
        ...
        if force_mask:
            return _stop_fuse_list(self.trans_forward(ids_lst, cond_vector, cond_word_vector, padding_mask, force_mask=True))

        logits_lst = self.trans_forward(ids_lst, cond_vector, cond_word_vector, padding_mask)
        if cond_scale == 1:
            return _stop_fuse_list(logits_lst)
        ...
        for logits, aux_logits in zip(logits_lst, aux_logits_lst):
            scaled_logits = aux_logits + (logits - aux_logits) * cond_scale
            scaled_logits_lst.append(scaled_logits.stop_fuse())
    ...
    def generate(
        ...
        if self.cond_mode == 'text':
            ...
            cond_vector = jt.zeros(batch_size, self.latent_dim).float().to(device)
            cond_word_vector = jt.zeros(batch_size, 77, self.latent_dim).float().to(device)
        else:
            raise NotImplementedError("Unsupported condition mode!!!")
        ...
        ids_lst, scores_lst = [], []
        for _ in range(3):
            ids = jt.where(part_padding_mask, self.pad_id, self.mask_id).to(device)
            scores = jt.where(part_padding_mask, 1e5, 0.).to(device)
        ...
        for timestep in jt.linspace(0, 1, timesteps, device=device):
            # 0 < timestep < 1
            ...
            num_token_masked = jt.round(rand_mask_prob * part_m_lens).clamp(min=1)  # (b, )
            ...
            for k, scores in enumerate(scores_lst):
                ...
                ids_lst[k] = jt.where(is_mask, self.mask_id, ids_lst[k])
            ...
            for k, logits in enumerate(logits_lst):
                logits = logits.stop_fuse()
                # print(logits.shape, self.num_tokens)
                # clean low prob token
                filtered_logits = top_k(logits, topk_filter_thres, dim=-1).stop_fuse()
                '''
                Update ids
                '''
                ...
                if gsample:  # use gumbel_softmax sampling
                    pred_ids = gumbel_sample(filtered_logits, temperature=temperature, dim=-1).stop_fuse()  # (b, seqlen)
                else:  # use multinomial sampling
                    probs = F.softmax(filtered_logits / temperature, dim=-1).stop_fuse()  # (b, seqlen, ntoken)
                    pred_ids = Categorical(probs).sample().stop_fuse()  # (b, seqlen)

                ids_lst[k] = jt.where(is_mask_lst[k], pred_ids, ids_lst[k]).stop_fuse()
                '''
                Updating scores
                '''
                probs_without_temperature = logits.softmax(dim=-1).stop_fuse()  # (b, seqlen, ntoken)
                ...
                scores_lst[k] = scores_lst[k].masked_fill(~is_mask_lst[k], 1e5).stop_fuse()

            # Jittor is lazy; flush each denoising step to avoid retaining the whole generation graph.
            jt.sync_all()
            jt.gc()

        for i in range(len(ids_lst)):
            ids_lst[i] = jt.where(part_padding_mask, self.pad_id, ids_lst[i])
    ...
    def edit(
        ...
        motion1_ids = jt.where(part_edit_mask, self.mask_id, motion1_ids)
        motion1_ids = jt.where(part_padding_mask, self.pad_id, motion1_ids)
        motion1_scores = jt.where(part_edit_mask, 0., 1e5)
        motion1_scores = jt.where(part_padding_mask, 1e5, motion1_scores)

        motion2_ids = motion2_ids.reshape(-1, part_padding_mask.shape[1])
        motion2_ids = jt.where(part_edit_mask, self.mask_id, motion2_ids)
        motion2_ids = jt.where(part_padding_mask, self.pad_id, motion2_ids)
        motion2_scores = jt.where(part_edit_mask, 0., 1e5)
        motion2_scores = jt.where(part_padding_mask, 1e5, motion2_scores)
        
        dist_ids = dist_ids.reshape(-1, part_padding_mask.shape[1])
        dist_ids = jt.where(part_edit_mask, self.mask_id, dist_ids)
        dist_ids = jt.where(part_padding_mask, self.pad_id, dist_ids)
        dist_scores = jt.where(part_edit_mask, 0., 1e5)
        dist_scores = jt.where(part_padding_mask, 1e5, dist_scores)
        ...
        for timestep in jt.linspace(0, 1, timesteps, device=device):
            ...
            for k, logits in enumerate(logits_lst):
                ...
                ids_lst[k] = jt.where(is_mask_lst[k], pred_ids, ids_lst[k])
                '''
                Updating scores
                '''
    ...
    def react(
        ...
        for i in range(3):
            if i == 0:
                motion1_ids = motion1_ids.reshape(-1, part_padding_mask.shape[1])
                ids = jt.where(part_padding_mask, self.pad_id, motion1_ids).to(device)
                scores = jt.where(part_padding_mask, 1e5, 1e5).to(device)
            else:
                ids = jt.where(part_padding_mask, self.pad_id, self.mask_id).to(device)
                scores = jt.where(part_padding_mask, 1e5, 0.).to(device)
            ids_lst.append(ids)  # (b*5, t)
