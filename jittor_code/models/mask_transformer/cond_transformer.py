...
from utils.jittor_compat import Tensor, nn
...
class SelfAttnBlock(nn.Module):
    ...
    def execute(self, src, shift, scale, gate, src_key_padding_mask=None, pre_normalize=True):
        if pre_normalize:
...
class CrossAttnBlock(nn.Module):
    ...
    def execute(self, tgt, memory, shift, scale, gate, memory_key_padding_mask=None, pre_normalize=True):
        if pre_normalize:
...
class FFN(nn.Module):
    ...
    def execute(self, src, shift, scale, gate, pre_normalize=True):
        if pre_normalize:
...
class TextCondTransformerDecoderLayer(nn.Module):
    ...
    def execute(self, x_ids, sent, words, tgt_key_padding_mask):
        '''
...
class MotCondTransformerDecoderLayer(nn.Module):
    ...
    def execute(self, x_ids, cond_tag, tgt_key_padding_mask, eps=1e-6):
        '''
        ...
        for i, data, cond_mot in zip([0, 1, 2], [mot1, mot2, dist], [[mot2_ex, dist_ex], [mot1_ex, dist_ex], [mot1_ex, mot2_ex]]):
            ...
            routing_weights = routing_weights.unsqueeze(-1)  # (bs, seq, 2, 1)

            cond_mot = jt.stack(cond_mot, dim=2)  # (bs, seq, 2, dim)
        ...
        x_ids = jt.stack(output_lst, dim=1)  # (bs, 3, seq, dim)
        return x_ids
