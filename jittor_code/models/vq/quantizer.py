...
class QuantizeEMAReset(nn.Module):
    ...
    def reset_codebook(self):
        ...
        self.register_buffer('codebook', jt.zeros(self.nb_code, self.code_dim, requires_grad=False).cuda())
    ...
    def init_codebook(self, x):
        ...
        self.code_count = jt.ones(self.nb_code, device=self.codebook.device)
        self.init = True
    ...
    def quantize(self, x, sample_codebook_temp=0.):
        ...
        # x: NT X C
        # NT X N
        distance = jt.sum(x ** 2, dim=-1, keepdim=True) - \
                   2 * jt.matmul(x, k_w) + \
                   jt.sum(k_w ** 2, dim=0, keepdim=True)  # (N * L, b)

        # code_idx = jt.argmin(distance, dim=-1)
    ...
    def compute_perplexity(self, code_idx):
        ...
        code_onehot = jt.zeros(self.nb_code, code_idx.shape[0], device=code_idx.device)  # nb_code, N * L
        code_onehot.scatter_(0, code_idx.view(1, code_idx.shape[0]), 1)

        code_count = code_onehot.sum(dim=-1)  # nb_code
        prob = code_count / jt.sum(code_count)
        perplexity = jt.exp(-jt.sum(prob * jt.log(prob + 1e-7)))
        return perplexity

    @jt.no_grad()
    def update_codebook(self, x, code_idx):
        code_onehot = jt.zeros(self.nb_code, x.shape[0], device=x.device)  # nb_code, N * L
        code_onehot.scatter_(0, code_idx.view(1, x.shape[0]), 1)

        code_sum = jt.matmul(code_onehot, x)  # nb_code, c
        code_count = code_onehot.sum(dim=-1)  # nb_code
    ...
    def execute(self, x, return_idx=False, temperature=0.):
        N, width, T = x.shape
