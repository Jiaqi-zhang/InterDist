...
class VQVAE(nn.Module):
    ...
    def preprocess(self, x, joint_dist):
        if self.dataname == "InterHuman":
            ...
            rot = jt.cat([jt.zeros(bs, seq, 1, 6).to(x.device), rot], dim=2)
            feet = x[:, :, -4:].reshape([bs, seq, 1, 4]).repeat(1, 1, self.joints_num, 1)
            joints = jt.cat([pos, vel, rot, feet], dim=-1)  # B, T, 22, 3+3+6+4=16            
    ...
    def postprocess(self, mot1, mot2):
        ...
        def reformat(x):
            ...
            x = jt.cat([pos, vel, rot, feet], dim=-1)
            return x
    ...
    def forward_decoder(self, mot1_cidx, mot2_cidx, dist_cidx, num_seq, soft_lookup=False):
        ...
        if not soft_lookup:
            mot1 = self.quantizer.get_codes_from_indices(mot1_cidx.unsqueeze(-1))[0]
            mot2 = self.quantizer.get_codes_from_indices(mot2_cidx.unsqueeze(-1))[0]
            joint_dist = self.quantizer_dist.get_codes_from_indices(dist_cidx.unsqueeze(-1))[0]
