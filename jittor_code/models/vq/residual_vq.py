...
class ResidualVQ(nn.Module):
    ...
    def codebooks(self):
        ...
        codebooks = jt.stack(codebooks, dim=0)
    ...
    def get_codes_from_indices(self, indices):  #indices shape 'b n q' # dequantize
        ...
        codebooks = self.codebooks.unsqueeze(1).repeat(1, batch, 1, 1)
        gather_indices = indices.permute(2, 0, 1).unsqueeze(-1).repeat(1, 1, 1, codebooks.shape[-1])
    ...
    def get_soft_codes_from_probs(self, probs):
        ...
        all_soft_codes = jt.zeros((self.num_quantizers, *probs.shape[:2], self.codebooks.shape[-1]), device=probs.device)

        for i in range(self.num_quantizers):
            all_soft_codes[i] = jt.matmul(probs[:, :, i, :], self.codebooks[i])
    ...
    def get_codebook_entry(self, indices):  #indices shape 'b n q'
        ...
        latent = jt.sum(all_codes, dim=0)  #'b n d'
        latent = latent.permute(0, 2, 1)
        return latent

    def execute(self, x, return_all_codes=False, sample_codebook_temp=None, force_dropout_index=-1):        
        ...
        if should_quantize_dropout:
            ...
            null_indices = jt.full(null_indices_shape, -1., device=device, dtype=jt.long)
            # null_loss = 0.
        ...
        all_indices = jt.stack(all_indices, dim=-1)
    ...
    def quantize(self, x, return_latent=False):
        ...
        code_idx = jt.stack(all_indices, dim=-1)
        all_codes = jt.stack(all_codes, dim=0)
        if return_latent:
