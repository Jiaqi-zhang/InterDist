...
from utils.jittor_graph import SAGEConv
...
class Decoder(nn.Module):
    ...
    def execute(self, mot1, mot2, dist, input_size):
        '''
        mot1: (bs, 32, J, T//4)
        mot2: (bs, 32, J, T//4)
        dist: (bs, 32, J, T//4)
        ...
        mot1 = jt.cat([dist, mot1], dim=1)
        mot2 = jt.cat([dist, mot2], dim=1)
        mot = jt.cat((mot1, mot2), dim=0)  # (bs*2, 32*2, J, T//4)
        ...
        x = jt.cat((mot1, mot2), dim=0)  # (bs*2*J, T//4, 512)
