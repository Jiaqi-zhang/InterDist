...
from utils.quaternion import cont6d_to_matrix, qbetween
...
class Geometric_Losses:
    ...
    def __init__(self, norm_data_root, recons_loss, joints_num, dataset_name, device, ex_loss=False):
        ...
        if recons_loss == 'l1':
            self.l1_criterion = jt.nn.L1Loss()
        elif recons_loss == 'l1_smooth':
            self.l1_criterion = jt.nn.SmoothL1Loss()
    ...
    def calc_foot_contact(self, motion, pred_motion):
        ...
        ## Calculate contacts
        thres = 0.001
        velfactor, heightfactor = jt.Tensor([thres, thres, thres, thres]).to(feet_vel.device), jt.Tensor([0.12, 0.05, 0.12, 0.05]).to(feet_vel.device)
        ...
        fc_loss = self.l1_criterion(pred_feet_vel[contact], jt.zeros_like(pred_feet_vel)[contact])
        if jt.isnan(fc_loss):
            fc_loss = jt.tensor(0).to(motion.device)
            if contact.sum() != 0:
    ...
    def calc_loss_geo(self, pred_rot, gt_rot, eps=1e-7):
        ...
        m = jt.bmm(gt_m, pred_m.transpose(1, 2))  #batch*3*3

        cos = (m[:, 0, 0] + m[:, 1, 1] + m[:, 2, 2] - 1) / 2
        theta = jt.acos(jt.clamp(cos, -1 + eps, 1 - eps))

        return jt.mean(theta)
    ...
    def forward(self, motions, pred_motion, only_rec=False):
        ...
        if self.dataset_name == 'InterHuman':            
            ...
            loss_bn = jt.zeros_like(loss_rec)            
