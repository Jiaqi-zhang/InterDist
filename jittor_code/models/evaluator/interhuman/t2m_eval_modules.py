...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.0, max_len=5000):
        ...
        div_term = jt.exp(jt.arange(0, d_model, 2).float() * (-np.log(10000.0) / d_model))
...
class MotionEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.query_token = nn.Parameter(jt.randn(1, self.latent_dim))
    ...
    def execute(self, batch):
        ...
        emb = jt.cat([self.query_token[jt.zeros(B, dtype=jt.long, device=x.device)][:, None], x_emb], dim=1)

        seq_mask = (mask > 0.5)
        token_mask = jt.ones((B, 1), dtype=bool, device=x.device)
        valid_mask = jt.cat([token_mask, seq_mask], dim=1)
...
class InterCLIP(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        clip_model, _ = clip.load("ViT-L/14@336px", device="cpu", jit=False, warn=False)

        self.token_embedding = clip_model.token_embedding
        self.positional_embedding = clip_model.positional_embedding
        self.dtype = clip_model.dtype
        self.latent_scale = nn.Parameter(jt.Tensor([1]))

        set_requires_grad(self.token_embedding, False)

        textTransEncoderLayer = nn.TransformerEncoderLayer(d_model=768, nhead=8, dim_feedforward=cfg.FF_SIZE, dropout=0.1, activation="gelu", batch_first=True)
        self.textTransEncoder = nn.TransformerEncoder(textTransEncoderLayer, num_layers=8)
        self.text_ln = nn.LayerNorm(768)
        self.out = nn.Linear(768, 512)

        self.clip_training = "text_"
        self.l1_criterion = jt.nn.L1Loss(reduction='mean')
    ...
    def compute_clip_losses(self, batch):
        ...
        if 1:
            for d in self.clip_training.split('_')[:1]:
                ...
                ground_truth = jt.arange(batch_size, dtype=jt.long, device=motion_features.device)
    ...
    def generate_src_mask(self, T, length):
        B = length.shape[0]
        src_mask = jt.ones(B, T)
        for i in range(B):
            cur_len = int(length[i].item()) if hasattr(length[i], "item") else int(length[i])
            for j in range(cur_len, T):
                src_mask[i, j] = 0
        return src_mask
    ...
    def encode_text(self, batch):
        ...
        eot_indices = [int(v) for v in text.argmax(dim=-1).numpy().tolist()]
        out = jt.cat([out[i, idx].unsqueeze(0) for i, idx in enumerate(eot_indices)], dim=0)
        out = self.out(out)
