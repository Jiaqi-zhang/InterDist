...
"""
pi = jt.Tensor([3.14159265358979323846])
...
def rad2deg(tensor):
    ...
    Example:
        >>> input = tgm.pi * jt.rand(1, 3, 3)
        >>> output = tgm.rad2deg(input)
    """
    if not jt.is_tensor(tensor):
        raise TypeError("Input type is not a jt.Tensor. Got {}"
                        .format(type(tensor)))
...
def convert_points_from_homogeneous(points):
    ...
    Examples::
        ...
        >>> input = jt.rand(2, 4, 3)  # BxNx3
        >>> output = tgm.convert_points_from_homogeneous(input)  # BxNx2
    """
    if not jt.is_tensor(points):
        raise TypeError("Input type is not a jt.Tensor. Got {}".format(
            type(points)))
...
def angle_axis_to_rotation_matrix(angle_axis):
    ...
    def _compute_rotation_matrix(angle_axis, theta2, eps=1e-6):
        ...
        k_one = 1.0
        theta = jt.sqrt(theta2)
        wxyz = angle_axis / (theta + eps)
        wx, wy, wz = jt.chunk(wxyz, 3, dim=1)
        cos_theta = jt.cos(theta)
        sin_theta = jt.sin(theta)
        ...
        rotation_matrix = jt.cat(
            [r00, r01, r02, r10, r11, r12, r20, r21, r22], dim=1)
        return rotation_matrix.view(-1, 3, 3)

    def _compute_rotation_matrix_taylor(angle_axis):
        rx, ry, rz = jt.chunk(angle_axis, 3, dim=1)
        k_one = jt.ones_like(rx)
        rotation_matrix = jt.cat(
    ...
    _angle_axis = jt.unsqueeze(angle_axis, dim=1)
    theta2 = jt.matmul(_angle_axis, _angle_axis.transpose(1, 2))
    theta2 = jt.squeeze(theta2, dim=1)
    ...
    rotation_matrix = jt.eye(4).to(angle_axis.device).type_as(angle_axis)
...
def rotation_matrix_to_quaternion(rotation_matrix, eps=1e-6):
    ...
    q0 = jt.stack([rmat_t[:, 1, 2] - rmat_t[:, 2, 1],
                      t0, rmat_t[:, 0, 1] + rmat_t[:, 1, 0],
                      rmat_t[:, 2, 0] + rmat_t[:, 0, 2]], -1)
    t0_rep = t0.repeat(4, 1).t()

    t1 = 1 - rmat_t[:, 0, 0] + rmat_t[:, 1, 1] - rmat_t[:, 2, 2]
    q1 = jt.stack([rmat_t[:, 2, 0] - rmat_t[:, 0, 2],
                      rmat_t[:, 0, 1] + rmat_t[:, 1, 0],
                      t1, rmat_t[:, 1, 2] + rmat_t[:, 2, 1]], -1)
    t1_rep = t1.repeat(4, 1).t()

    t2 = 1 - rmat_t[:, 0, 0] - rmat_t[:, 1, 1] + rmat_t[:, 2, 2]
    q2 = jt.stack([rmat_t[:, 0, 1] - rmat_t[:, 1, 0],
                      rmat_t[:, 2, 0] + rmat_t[:, 0, 2],
                      rmat_t[:, 1, 2] + rmat_t[:, 2, 1], t2], -1)
    t2_rep = t2.repeat(4, 1).t()

    t3 = 1 + rmat_t[:, 0, 0] + rmat_t[:, 1, 1] + rmat_t[:, 2, 2]
    q3 = jt.stack([t3, rmat_t[:, 1, 2] - rmat_t[:, 2, 1],
                      rmat_t[:, 2, 0] - rmat_t[:, 0, 2],
                      rmat_t[:, 0, 1] - rmat_t[:, 1, 0]], -1)
    t3_rep = t3.repeat(4, 1).t()

    mask_c0 = mask_d2 * mask_d0_d1
    mask_c1 = mask_d2 * jt.logical_not(mask_d0_d1)
    mask_c2 = jt.logical_not(mask_d2) * mask_d0_nd1
    mask_c3 = jt.logical_not(mask_d2) * jt.logical_not(mask_d0_nd1)
    ...
    q /= jt.sqrt(t0_rep * mask_c0 + t1_rep * mask_c1 +  # noqa
                    t2_rep * mask_c2 + t3_rep * mask_c3)  # noqa
    q *= 0.5
    return q


def quaternion_to_angle_axis(quaternion) -> jt.Tensor:
    """Convert quaternion vector to angle axis of rotation.

    Adapted from ceres C++ library: ceres-solver/include/ceres/rotation.h

    Args:
        quaternion (jt.Tensor): tensor with quaternions.

    Return:
        jt.Tensor: tensor with angle axis of rotation.
    ...
    Example:
        >>> quaternion = jt.rand(2, 4)  # Nx4
        >>> angle_axis = tgm.quaternion_to_angle_axis(quaternion)  # Nx3
    """
    if not jt.is_tensor(quaternion):
        raise TypeError("Input type is not a jt.Tensor. Got {}".format(
            type(quaternion)))
    ...
    sin_theta = jt.sqrt(sin_squared_theta)
    cos_theta = quaternion[..., 0]
    two_theta = 2.0 * jt.where(
        cos_theta < 0.0,
        jt.atan2(-sin_theta, -cos_theta),
        jt.atan2(sin_theta, cos_theta))

    k_pos = two_theta / sin_theta
    k_neg = 2.0 * jt.ones_like(sin_theta)
    k = jt.where(sin_squared_theta > 0.0, k_pos, k_neg)

    angle_axis = jt.zeros_like(quaternion)[..., :3]
    angle_axis[..., 0] += q1 * k
    angle_axis[..., 1] += q2 * k
    angle_axis[..., 2] += q3 * k
    return angle_axis
...
def angle_axis_to_quaternion(angle_axis) -> jt.Tensor:
    """Convert an angle axis to a quaternion.

    Adapted from ceres C++ library: ceres-solver/include/ceres/rotation.h

    Args:
        angle_axis (jt.Tensor): tensor with angle axis.

    Return:
        jt.Tensor: tensor with quaternion.
    ...
    Example:
        >>> angle_axis = jt.rand(2, 4)  # Nx4
        >>> quaternion = tgm.angle_axis_to_quaternion(angle_axis)  # Nx3
    """
    if not jt.is_tensor(angle_axis):
        raise TypeError("Input type is not a jt.Tensor. Got {}".format(
            type(angle_axis)))
    ...
    theta = jt.sqrt(theta_squared)
    half_theta = theta * 0.5

    mask = theta_squared > 0.0
    ones = jt.ones_like(half_theta)

    k_neg = 0.5 * ones
    k_pos = jt.sin(half_theta) / theta
    k = jt.where(mask, k_pos, k_neg)
    w = jt.where(mask, jt.cos(half_theta), ones)

    quaternion = jt.zeros_like(angle_axis)
    quaternion[..., 0:1] += a0 * k
    quaternion[..., 1:2] += a1 * k
    quaternion[..., 2:3] += a2 * k
    return jt.cat([w, quaternion], dim=-1)
