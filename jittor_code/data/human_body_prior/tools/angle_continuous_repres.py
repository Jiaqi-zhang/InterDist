...
def bgs(d6s):
    ...
    a2 = d6s[:,:,1]
    c = jt.bmm(b1.view(bsz,1,-1),a2.view(bsz,-1,1)).view(bsz,1)*b1
    b2 = F.normalize(a2-c,p=2,dim=1)
    b3=jt.cross(b1,b2,dim=1)
    return jt.stack([b1,b2,b3],dim=1).permute(0,2,1)
...
class geodesic_loss_R(nn.Module):
    ...
    def bgdR(self,m1,m2):
        batch = m1.shape[0]
        m = jt.bmm(m1, m2.transpose(1, 2))  # batch*3*3

        cos = (m[:, 0, 0] + m[:, 1, 1] + m[:, 2, 2] - 1) / 2
        cos = jt.min(cos, m1.new(np.ones(batch)))
        cos = jt.max(cos, m1.new(np.ones(batch)) * -1)

        return jt.acos(cos)

    def execute(self, ypred, ytrue):
        theta = self.bgdR(ypred,ytrue)
        if self.reduction == 'mean':
            return jt.mean(theta)
        if self.reduction == 'batchmean':
            breakpoint()
            return jt.mean(jt.sum(theta, dim=theta.shape[1:]))
