...
class AdamInClosure():
    def __init__(self, var_list, lr, max_iter=100, tolerance_change=1e-5):
        self.optimizer = jt.optim.Adam(var_list, lr)
...
def ik_fit(optimizer, source_kpts_model, static_vars, vp_model, extra_params={}, on_step=None, gstep=0):
    ...
    data_loss = extra_params.get('data_loss', jt.nn.SmoothL1Loss(reduction='mean'))
    # data_loss =
    # data_loss = jt.nn.L1Loss(reduction='mean')#change with SmoothL1

    def fit(weights, free_vars):

        fit.gstep += 1
        free_vars['pose_body'] = vp_model.decode(free_vars['poZ_body'])['pose_body'].contiguous().view(-1, 63)
        nonan_mask = jt.isnan(free_vars['poZ_body']).sum(-1) == 0
        ...
        opt_objs['betas'] = jt.pow(free_vars['betas'][nonan_mask],2).sum()
        opt_objs['poZ_body'] = jt.pow(free_vars['poZ_body'][nonan_mask],2).sum()


        opt_objs = {k: opt_objs[k]*v for k, v in weights.items() if k in opt_objs.keys()}
        loss_total = jt.sum(jt.stack(list(opt_objs.values())))
        # breakpoint()
...
class IK_Engine(nn.Module):
    ...
    def __init__(self,
        ...
        :param data_loss: should be a Jittor callable (source, target) that returns the accumulated loss
        ...
        self.data_loss = jt.nn.SmoothL1Loss(reduction='mean') if data_loss is None else data_loss
    ...
    def execute(self, source_kpts, target_kpts, initial_body_params={}):
        '''
        ...
        if 'pose_body' not in initial_body_params:
            initial_body_params['pose_body'] = jt.zeros([bs, 63], device=comp_device, dtype=jt.float, requires_grad=False)
        if 'trans' not in initial_body_params:
            initial_body_params['trans'] = jt.zeros([bs, 3], device=comp_device, dtype=jt.float, requires_grad=False)
        if 'betas' not in initial_body_params:
            initial_body_params['betas'] = jt.zeros([bs, 10], device=comp_device, dtype=jt.float, requires_grad=False)
        if 'root_orient' not in initial_body_params:
            initial_body_params['root_orient'] = jt.zeros([bs, 3], device=comp_device, dtype=jt.float, requires_grad=False)

        initial_body_params['poZ_body'] = self.vp_model.encode(initial_body_params['pose_body']).mean

        free_vars = {k: jt.nn.Parameter(v.detach(), requires_grad=True) for k,v in initial_body_params.items() if k in ['betas', 'trans', 'poZ_body', 'root_orient']}
        static_vars = {
        ...
        if self.optimizer_args['type'].upper() == 'LBFGS':
            optimizer = jt.optim.LBFGS(list(free_vars.values()),
        ...
        # if closure.final_loss is None or jt.isnan(closure.final_loss) or jt.any(jt.isnan(free_vars['trans'])):
        #     if self.verbosity > 0:
