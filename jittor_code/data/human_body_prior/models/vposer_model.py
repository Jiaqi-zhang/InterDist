...
class ContinousRotReprDecoder(nn.Module):
    ...
    def execute(self, module_input):
        reshaped_input = module_input.view(-1, 3, 2)

        b1 = F.normalize(reshaped_input[:, :, 0], dim=1)

        dot_prod = jt.sum(b1 * reshaped_input[:, :, 1], dim=1, keepdim=True)
...
class NormalDistDecoder(nn.Module):
    ...
    def execute(self, Xout):
        return jt.distributions.normal.Normal(self.mu(Xout), F.softplus(self.logvar(Xout)))
...
class VPoser(nn.Module):
    ...
    def sample_poses(self, num_poses, seed=None):
        ...
        self.eval()
        with jt.no_grad():
            Zgen = jt.tensor(np.random.normal(0., 1., size=(num_poses, self.latentD)), dtype=dtype, device=device)
