...
class _UnavailableTrainer:
    def __init__(self, *args, **kwargs):
        raise RuntimeError("VPoser PyTorch-Lightning training is not ported to Jittor.")
...
class DDPPlugin(_NoopCallback):
    pass


LearningRateMonitor = EarlyStopping = ModelCheckpoint = TensorBoardLogger = _NoopCallback
pl = types.SimpleNamespace(Trainer=_UnavailableTrainer)
...
from utils.jittor_compat import optim as optim_module
from utils.jittor_compat import lr_scheduler as lr_sched_module
from utils.jittor_data import DataLoader
...
class VPoserTrainer(LightningModule):
    ...
    def __init__(self, _config):
        ...
        self.example_input_array = {'pose_body':jt.ones(vp_ps.train_parms.batch_size, 63),}
        self.vp_ps = vp_ps

    def execute(self, pose_body):
    ...
    def _compute_loss(self, dorig, drec):
        l1_loss = jt.nn.L1Loss(reduction='mean')
        ...
        # q_z = jt.distributions.normal.Normal(drec['mean'], drec['std'])
        q_z = drec['q_z']
        # dorig['fullpose'] = jt.cat([dorig['root_orient'], dorig['pose_body']], dim=-1)
        ...
        # KL loss
        p_z = jt.distributions.normal.Normal(
            loc=jt.zeros((bs, latentD), device=device, requires_grad=False),
            scale=jt.ones((bs, latentD), device=device, requires_grad=False))
        weighted_loss_dict = {
            'loss_kl':loss_kl_wt * jt.mean(jt.sum(jt.distributions.kl.kl_divergence(q_z, p_z), dims=[1])),
        ...
        weighted_loss_dict['loss_total'] = jt.stack(list(weighted_loss_dict.values())).sum()

        with jt.no_grad():
            unweighted_loss_dict = {'v2v': jt.sqrt(jt.pow(bm_rec.v-bm_orig.v, 2).sum(-1)).mean()}
            unweighted_loss_dict['loss_total'] = jt.cat(
    ...
    def validation_epoch_end(self, outputs):
        ...
        metrics = {k: jt.as_tensor(v) for k, v in metrics.items()}
