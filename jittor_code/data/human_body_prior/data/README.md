...
This legacy VPoser data-preparation note described the original PyTorch pipeline. In this Jittor port, use it only as historical reference for the AMASS-to-HDF5 data stages:
...
***Stage II*** turns the AMASS intermediate files into HDF5, *h5* files and along the process augments the data with extra fields or noise. 
The middle stage parallelizes augmentation tasks. 
Furthermore, we use HDF5 files for the middle stage so that they can be used in other deep learning frameworks as well.

***Stage III*** again converts the augmented HDF5 files into final dataset files that should be provided to the current VPoser training script.
