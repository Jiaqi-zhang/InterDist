...
def to_tensor(array, dtype=jt.float32):
    if 'jt.tensor' not in str(type(array)):
        return jt.tensor(array, dtype=dtype)
...
def find_dynamic_lmk_idx_and_bcoords(vertices, pose, dynamic_lmk_faces_idx,
    ...
    rel_rot_mat = jt.eye(3, device=vertices.device,
...
def vertices2landmarks(vertices, faces, lmk_faces_idx, lmk_bary_coords):
    ...
    lmk_faces += jt.arange(
        batch_size, dtype=jt.long, device=device).view(-1, 1, 1) * num_verts
...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
        lbs_weights, joints = None, pose2rot=True, v_shaped=None, dtype=jt.float32):
    ...
    # N x J x 3 x 3
    ident = jt.eye(3, dtype=dtype, device=device)
    if pose2rot:
        rot_mats = batch_rodrigues(
    ...
    v_posed_homo = jt.cat([v_posed, homogen_coord], dim=2)
...
def batch_rodrigues(rot_vecs, epsilon=1e-8, dtype=jt.float32):
    ...
    K = jt.zeros((batch_size, 3, 3), dtype=dtype, device=device)

    zeros = jt.zeros((batch_size, 1), dtype=dtype, device=device)
    K = jt.cat([zeros, -rz, ry, rz, zeros, -rx, -ry, rx, zeros], dim=1) \
        .view((batch_size, 3, 3))

    ident = jt.eye(3, dtype=dtype, device=device).unsqueeze(dim=0)
