...
from utils.misc import to_jittor
...
def parse_motion_file(motion_file):
    ...
    with jt.no_grad():
        for fId in range(0, frame_number):
            root_orient = jt.Tensor(global_orient[fId:fId + 1, :]).to(comp_device)
            pose_body = jt.Tensor(body_pose[fId:fId + 1, :].reshape([1, -1])).to(comp_device)
            hand_pose = np.concatenate([left_hand_pose[fId:fId + 1, :], right_hand_pose[fId:fId + 1, :]], axis=1).reshape([1, -1])
            pose_hand = jt.Tensor(hand_pose).to(comp_device)
            trans = jt.Tensor(root_transl[fId:fId + 1]).to(comp_device)
    ...
    pose_seq = jt.cat(pose_seq, dim=0)
...
def forward_kinematics(pose, root_pos, kinematic_tree, skel_joints, do_root_R=True):
    T, J, _ = pose.shape
    pose = to_jittor(pose)
    root_pos = to_jittor(root_pos)
    pose_mat = geometry.axis_angle_to_matrix(pose)
    offsets = jt.from_numpy(skel_joints).to(pose_mat.device)

    if len(offsets.shape) == 2:
        offsets = offsets.expand(T, -1, -1)
    joints = jt.zeros((T, J, 3)).to(pose_mat.device)
    joints[..., 0, :] = root_pos
    for chain in kinematic_tree:
        if do_root_R:
            matR = pose_mat[:, 0]
        else:
            matR = jt.eye(3).expand((len(T), -1, -1)).detach().to(pose_mat.device)
        for i in range(1, len(chain)):
            matR = jt.matmul(matR, pose_mat[:, chain[i]])
            offset_vec = offsets[:, chain[i]].unsqueeze(-1)
            joints[:, chain[i]] = jt.matmul(matR, offset_vec).squeeze(-1) + joints[:, chain[i - 1]]
    return joints
...
def forward_kinematics_smplx(data):
    ...
    with jt.no_grad():
        for fId in range(0, frame_number):
            root_orient = jt.Tensor(rotations[fId:fId + 1, 0, :]).to(comp_device)
            pose_body = jt.Tensor(rotations[fId:fId + 1, 1:22, :].reshape([1, -1])).to(comp_device)
            pose_hand = jt.Tensor(rotations[fId:fId + 1, 25:, :].reshape([1, -1])).to(comp_device)
            trans = jt.Tensor(root_transl[fId:fId + 1]).to(comp_device)
...
def normalize_motion_pose(motion, gpos_prev_frame, face_joint_indx=[2, 1, 17, 16]):
    ...
    quant_rotations = geometry.axis_angle_to_quaternion(to_jittor(rotations))
    quant_rotations[:, 0] = qmul(to_jittor(root_quat_init_for_all).double(), quant_rotations[:, 0])
...
if __name__ == '__main__':
    ...
    comp_device = jt.device("cuda:0" if jt.cuda.is_available() else "cpu")
    num_betas = 10
    downsample = 4
