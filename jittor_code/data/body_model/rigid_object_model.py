...
class RigidObjectModel(nn.Module):
    ...
    def __init__(self, plpath, batch_size=1, dtype=jt.float32):
        super(RigidObjectModel, self).__init__()

        trans = jt.tensor(np.zeros((batch_size, 3)), dtype=dtype, requires_grad=True)
        self.register_parameter('trans', nn.Parameter(trans, requires_grad=True))

        root_orient = jt.tensor(np.zeros((batch_size, 3)), dtype=dtype, requires_grad=True)
        self.register_parameter('root_orient', nn.Parameter(root_orient, requires_grad=True))

        mesh = Mesh(filename=plpath)

        self.rigid_v = jt.from_numpy(np.repeat(mesh.v[np.newaxis], batch_size, axis=0)).type(dtype)
        self.f = jt.from_numpy(mesh.f.astype(np.int32))

    def execute(self, root_orient, trans):
        if root_orient is None: root_orient = self.root_orient
        if trans is None: trans = self.trans
        verts = jt.bmm(self.rigid_v, batch_rodrigues(root_orient)) + trans.view(-1,1,3)
