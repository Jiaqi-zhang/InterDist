...
def cont6d_to_matrix(cont6d, eps=1e-6):
    ...
    z_raw = torch.cross(x, y_raw, dim=-1)  
    parallel_mask = torch.norm(z_raw, dim=-1) < eps
    
    z = z_raw / (torch.norm(z_raw, dim=-1, keepdim=True) + 1e-12)  
    ...
    mat = torch.cat([x, y, z], dim=-1)  # [..., 3, 3]  
    
    eye3 = torch.eye(3, device=cont6d.device, dtype=cont6d.dtype)
    expand_shape = list(parallel_mask.shape) + [3, 3]  
    eye_expanded = eye3.view(*([1] * parallel_mask.ndim), 3, 3).expand(*expand_shape)
    mat = torch.where(parallel_mask.unsqueeze(-1).unsqueeze(-1), eye_expanded, mat)  
    return mat  
